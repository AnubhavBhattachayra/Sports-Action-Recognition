...
Creates a 20% subset of the precomputed RGB and Flow data while maintaining
the original directory structure. This is useful for testing and development.
"""
import os
import sys
import shutil
...
def create_subset(rgb_files, flow_files, output_base_dir, subset_fraction=0.2, seed=42):
    ...
    num_subset = int(num_files * subset_fraction)
...
def main():
    ...
    output_dir = r'C:\Users\anubh\OneDrive\Desktop\Thesis\precomputed_data_subset'
    ...
    # Create subset
    create_subset(rgb_files, flow_files, output_dir, subset_fraction=0.2)
    
    # Verify the subset
    subset_rgb_files, subset_flow_files = get_all_files(output_dir)
    print(f"\nVerification:")
    print(f"Original dataset: {len(rgb_files)} pairs")
    print(f"Subset dataset: {len(subset_rgb_files)} pairs")
    print(f"Subset percentage: {(len(subset_rgb_files) / len(rgb_files)) * 100:.1f}%")
